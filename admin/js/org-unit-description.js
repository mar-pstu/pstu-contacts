( function( blocks, editor, i18n, element, components, _ ) {
	...
	function set_shortcode( props, id ) {
		var shortcode = '[pstu_org_unit_description id="' + id + '" ]';
		props.setAttributes( { shortcode: shortcode } );
	}


	blocks.registerBlockType( 'pstu-contacts/org-unit-description', {
		title: i18n.__( 'Краткое описание подразделения', plugin_name ),
		description: '',
		keywords: [
		...
		edit: function( props ) {
			...
			return el( 'div', { className: props.className },
				...
				( props.attributes.id.length > 0 ) ? [
					...
					el( 'div', { className: 'org-unit-description' }, terms.find( x=>x.value==props.attributes.id ).description )
